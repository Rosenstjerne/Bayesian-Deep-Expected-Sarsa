...
class Agent:
    ...
    def process(self, state):
        #Maybe use torch for greyscale and rescale
        state = rgb2gray(state[35:195, :, :])
        state = rescale(state, scale=0.5)
        state = state[np.newaxis, np.newaxis, :, :] #What do?
    ...
    def play(self, episodes, train=False, load=False, plot=False, render=False, verbose=False):
        ...
        try:
            ...
            for episode in progress_bar:
                ...
                state = env.reset()
                #Creates Tensor/Frame of the state
                ...
                while not done:
                    ...
                    # Making sure we have in_channel number of frames
                    ...
                    if train and np.random.uniform() < self.exploration_rate(self.t-burn_in):
                        ...
                        action = self.act(state) #Action chosen by network
